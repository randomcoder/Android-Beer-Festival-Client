...
import uk.co.randomcoding.android.beerfestival.util.DialogueHelpers.alertDialogue
import uk.co.randomcoding.android.beerfestival.util.RestDownloader
 ...
 */
class UpdateDataActivity extends Activity {
  ...
  def updateData(view: View) {
    ...
    try {
      ...
      val downloadResult = loadDrinkData()
  ...
  private[this] def loadDrinkData(): Long = {
    ...
    // For testing just write a data file
    val dummyData = """"""

    RestDownloader.uriGet(dataUri, new FileOutputStream(dataFile))
  }
